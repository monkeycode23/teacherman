...
export function DashboardPage() {
  ...
  const totalStudents = classrooms.reduce((acc:any, c:any) => acc + c.students.length, 0);
  const totalAssignments = classrooms.reduce((acc:any, c:any) => acc + c.assignments.length, 0);
  const totalTopics = classrooms.reduce((acc:any, c:any) => acc + c.topics.length, 0);

  const pendingSubmissions = classrooms.reduce((acc:any, c:any) => {
    return acc + c.assignments.reduce((aAcc:any, a:any) => {
      return aAcc + a.submissions.filter((s:any) => s.status === 'pending').length;
    }, 0);
  }, 0);

  // Datos para gráfico de rendimiento por aula
  const classroomPerformance = classrooms.map((c:any) => ({
    name: c.name,
    promedio: c.students.length > 0
      ? Math.round(c.students.reduce((acc:any, s:any) => acc + s.averageGrade, 0) / c.students.length)
      : 0,
    asistencia: c.students.length > 0
      ? Math.round(c.students.reduce((acc:any, s:any) => acc + s.attendance, 0) / c.students.length)
      : 0,
  }));
  ...
  const studentDistribution = classrooms.map((c:any) => ({
    name: c.name,
          ...
          <ResponsiveContainer width="100%" height={250}>
            <PieChart>
              <Pie
                ...
                dataKey="value"
              >
                {studentDistribution.map((entry:any, index:any) => (
        ...
        <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6 lg:col-span-2">
          ...
          <div className="space-y-4">
            {classrooms.slice(0, 3).map((classroom:any) => {
              const recentComments = classroom.topics.flatMap((t:any) => t.comments).slice(0, 2);
