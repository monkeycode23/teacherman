...
import { CreateSessionSchema } from "../../errors/schemas/session.schema";
...
export default function CreateSessionModal() {
  ...
  useEffect(() => {
    ...
    formStore.setValue("date", new Date().toISOString().split("T")[0]);
    formStore.setValue("description", "");
    
    formStore.setValue("classroomId", classRoomStore.classroom._id);
    formStore.setValue("timeStart", "12:00");
    formStore.setValue("timeEnd", "13:00");
     ...
     CreateSessionSchema,
      (data: any) => {
        console.log("evnet", data);
            ...
            sessionStorage.addSession(data.data);
      ...
      <Modal open={showAddModal}>
        <form onSubmit={handleSubmit} className="  rounded-lg">
          <div className="flex items-center justify-between mb-4">
            <h3 className="text-gray-900">{"Nueva Session"}</h3>
            <button
              title="buton"
              type="button"
              /* nClick={handleCancel} */
          ...
          <div className="space-y-3">
            <input
              ...
              value={formStore.inputs["date"]}
              onChange={(e) => formStore.setValue("date", e.target.value)}
              className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-transparent"
            />

            {formStore.errors["date"] && (
              <p className="text-red-600"> {formStore.errors["date"]}</p>
            )}
